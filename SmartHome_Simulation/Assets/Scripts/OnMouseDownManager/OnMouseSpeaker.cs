...
public class OnMouseSpeaker : OnMouseManager
{
	/// <summary>
	/// Start this instance.
	/// </summary>
    ...
    void OnMouseDown()
    {
        if (!GameobjectLoader.isLoading())
        {
            if (Mode.isPlaceMode())
            {
                if (currentDeviceType.Equals(Config.STRING_TYPE_EN_SPEAKER))
                {
                    Mode.changeToPlaceSwitchMode();
                    SwitchPosition.currentObject = deviceTransform.gameObject;
